...
public class C_SpawnTest
{
    ...
    public IEnumerator CheckCorrectSpawn()
    {
        ...
        RaycastHit hit = PMHelper.RaycastFront(tmp.transform.position, Vector3.down, 1 << 16);
        if (!hit.collider)
        {
    ...
    //     {
    //         if (cube.transform.position.z != tmp.transform.position.z ||
    //             cube.transform.position.y != tmp.transform.position.y)
    //         {
